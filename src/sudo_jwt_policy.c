...
#define DEFAULT_CONFIG_PATH "/etc/sudo-jwt-policy.conf"
...
static int approval_show_version(int verbose) {
    if (verbose && g_printf) {
        g_printf(SUDO_CONV_INFO_MSG, "sudo-jwt-approval 0.1\n");
    }
    return 1;
}
...
cleanup:
    if (!ok && errstr && *errstr) {
        plugin_log(SUDO_CONV_ERROR_MSG, "sudo-jwt-approval: %s\n", *errstr);
    }
...
struct approval_plugin sudo_jwt_approval = {
